...
def hysteresis_class(x:pd.Series,y:pd.Series,x_fixed:pd.Series):
    ...
    # JB added 11/7/2025
    # figure out valid x_fixed range
    x_min_valid = x_norm.min()
    x_max_valid = x_norm.max()
    # trim x_fixed to only values that exist in x_norm
    x_fixed = x_fixed[(x_fixed >= x_min_valid) & (x_fixed <= x_max_valid)]
...
if __name__ == "__main__":
    ...
    # answer key to test this code against
    ...
    for sheet in range(4):
        ...
        # df has datetime index, Q in col[0] and dependent in col[1]
        ...
        x_fixed = pd.Series([0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 
                             ...
                             0.95, 1.00])  # Is this the stetp size/integral width?
